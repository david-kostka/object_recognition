...
flags.DEFINE_string('model_name', 'single_nao_test', 'The name of the exported trained model')
...
def train(model, trainset, valset):
    callbacks = [   #ReduceLROnPlateau(verbose=1),
                    ...
                    ModelCheckpoint(chkpts_path + FLAGS.model_name + '/' + FLAGS.model_name + '_{epoch}.tf', verbose=1, save_weights_only=True),
                    TensorBoard(log_dir=logs_path + FLAGS.model_name + '/' + '{}'.format(time())) ]
...
def save_model(model):
    model_path = models_path + '/' + FLAGS.model_name + '/'

    # Save Model as File
    model.save(model_path + FLAGS.model_name)

    # Save as Format compatible with CompiledNN
    model.save(model_path + FLAGS.model_name + '.hdf5', save_format='h5')

    # Convert and Save to TFLite Model
    converter = tf.lite.TFLiteConverter.from_saved_model(model_path + FLAGS.model_name)
    tflite_model = converter.convert()
    with open(model_path + FLAGS.model_name + '.tflite', 'wb') as f:
